...
class LibertyAudioRecordingController final : public juce::Component,
                                               private juce::AudioIODeviceCallback,
                                               private juce::Timer
{
public:
    explicit LibertyAudioRecordingController(MainComponent& ownerIn)
        : owner(ownerIn)
    {
...
private:
    ...
    bool configureInput()
    {
        ...
        {
            const auto available = static_cast<int>(device->getInputChannelNames().size());
            const auto wanted = juce::jmin(2, available);
            for (int i = 0; i < wanted; ++i)
                setup.inputChannels.setBit(i);

            if (wanted > 0 && manager.setAudioDeviceSetup(setup, true).isNotEmpty())
                return false;
        ...
        if (device != nullptr)
        {
            ...
            for (int i = 0; i < active.size() && inputIndices.size() < 2; ++i)
                if (active[i])
    ...
    void startRecording()
    {
        if (armedTrack < 0)
        {
            juce::AlertWindow::showMessageBoxAsync(
                ...
                "Liberty - Recording",
                "Arm an Audio track before pressing REC.",
                "OK");
            return;
        }
        ...
        if (!configureInput())
        {
            juce::AlertWindow::showMessageBoxAsync(
                juce::AlertWindow::WarningIcon,
                "Liberty - Recording",
                "No active input is available on the selected audio device.",
                "OK");
            return;
        }
        ...
        auto writer = std::unique_ptr<juce::AudioFormatWriter>(
            wav.createWriterFor(output.release(),
                                ...
                                (unsigned int)inputIndices.size(),
                                24,
                                {},
                                0));
        if (writer == nullptr)
                                          ...
                                          const juce::AudioIODeviceCallbackContext&) override
    {
        ...
        for (int channel = 0; channel < channels; ++channel)
        {
            ...
            if (inputChannelData != nullptr && inputChannelData[source] != nullptr)
...
public:
    ...
    ~LibertyAudioRecordingBootstrap() override
    {
        stopTimer();
        controllers.clear();
    }
