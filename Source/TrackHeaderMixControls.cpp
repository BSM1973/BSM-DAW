...
public:
 ...
 void paint(juce::Graphics&g)override{g.setFont(juce::Font(7.5f,juce::Font::bold));int rh=getLibertyTrackRowHeight();for(int i=0;i<(int)controls.size();++i){int logical=logicalTrack(i),y=76+rulerH+(logical-owner.getTrackScrollRows())*rh;if(y+rh<108||y>=owner.getHeight()-210)continue;int my=y+rh-30;g.setColour(juce::Colour(0xff1e232a));g.fillRect(6,my,200,30);g.setColour(juce::Colour(0xffc5cbd3));g.drawText("VOL",8,my+9,20,11,juce::Justification::centredLeft);g.setColour(juce::Colour(0xffffb04d));g.drawText("PAN",99,my+9,24,11,juce::Justification::centredLeft);}}
private:
 ...
 void ensure(){int wanted=owner.getAudioTrackCount()+owner.getInstrumentTrackCount();while((int)controls.size()<wanted){int i=(int)controls.size();Controls c;c.vol=std::make_unique<juce::Slider>();c.pan=std::make_unique<juce::Slider>();c.mute=std::make_unique<juce::TextButton>("M");c.solo=std::make_unique<juce::TextButton>("S");c.mask=std::make_unique<juce::Component>();auto* v=c.vol.get();v->setSliderStyle(juce::Slider::LinearHorizontal);v->setRange(0,2,.001);v->setTextBoxStyle(juce::Slider::NoTextBox,false,0,0);v->onValueChange=[this,i,v]{if(syncing)return;if(isAudio(i))owner.audioEngine.setTrackGain(i,(float)v->getValue());else setLibertyInstrumentGain((float)v->getValue());};auto*p=c.pan.get();p->setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);p->setRange(-1,1,.001);p->setTextBoxStyle(juce::Slider::NoTextBox,false,0,0);p->setLookAndFeel(&lf);p->onValueChange=[this,i,p]{if(syncing)return;if(isAudio(i))owner.audioEngine.setTrackPan(i,(float)p->getValue());else setLibertyInstrumentPan((float)p->getValue());};c.mute->onClick=[this,i]{if(isAudio(i))owner.audioEngine.setTrackMuted(i,!owner.audioEngine.isTrackMuted(i));else owner.audioEngine.setInstrumentTrackMuted(!owner.audioEngine.isInstrumentTrackMuted());};c.solo->onClick=[this,i]{if(isAudio(i))owner.audioEngine.setTrackSolo(i,!owner.audioEngine.isTrackSolo(i));else owner.audioEngine.setInstrumentTrackSolo(!owner.audioEngine.isInstrumentTrackSolo());};for(auto*b:{c.mute.get(),c.solo.get()}){b->setMouseClickGrabsKeyboardFocus(false);b->setColour(juce::TextButton::textColourOffId,juce::Colours::white);addAndMakeVisible(*b);}addAndMakeVisible(*v);addAndMakeVisible(*p);addAndMakeVisible(*c.mask);controls.push_back(std::move(c));}}
 ...
 void sync(){syncing=true;for(int i=0;i<(int)controls.size();++i){auto&c=controls[(size_t)i];float gain=isAudio(i)?owner.audioEngine.getTrackGain(i):getLibertyInstrumentGain();float pan=isAudio(i)?owner.audioEngine.getTrackPan(i):getLibertyInstrumentPan();c.vol->setValue(gain,juce::dontSendNotification);c.pan->setValue(pan,juce::dontSendNotification);bool m=isAudio(i)?owner.audioEngine.isTrackMuted(i):owner.audioEngine.isInstrumentTrackMuted(),s=isAudio(i)?owner.audioEngine.isTrackSolo(i):owner.audioEngine.isInstrumentTrackSolo();c.mute->setColour(juce::TextButton::buttonColourId,m?juce::Colour(0xff9b4545):juce::Colour(0xff31363e));c.solo->setColour(juce::TextButton::buttonColourId,s?juce::Colour(0xff8b7a32):juce::Colour(0xff31363e));}syncing=false;}
...
void toggleLibertyMidiInstrumentMute(MainComponent&owner){owner.audioEngine.setInstrumentTrackMuted(!owner.audioEngine.isInstrumentTrackMuted());owner.repaint();}
