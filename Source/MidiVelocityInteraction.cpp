...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        if (!inVelocityLane(*piano, p.y) || p.x < pianoKeyWidth) return;
        if (!selectNearestNote(*main, *piano, p.x)) return;
    ...
    {
        if (!editing || pianoRoll == nullptr) return;
...
private:
    static juce::Component* findPianoRoll(juce::Point<int> screenPosition) noexcept
    {
        ...
        while (component != nullptr)
        {
            if (auto* window = dynamic_cast<juce::DocumentWindow*>(component))
                if (window->getName() == "Liberty - MIDI 1") return window->getContentComponent();
    ...
    static bool selectNearestNote(MainComponent& main, const juce::Component& piano, int x)
    {
        ...
        const auto clipTicks = juce::jmax<std::int64_t>(1, MidiEngine::secondsToTick(main.getMidiClipLengthSeconds(), main.getTempoBpm()));

        bool found = false;
        MidiEngine::NoteEvent best{};
        double bestDistance = std::numeric_limits<double>::max();

        for (const auto& note : main.getMidiEngine().getNotesCopy())
        {
            if (note.startTick < 0 || note.startTick >= clipTicks) continue;
            const double noteX = static_cast<double>(pianoKeyWidth) + static_cast<double>(note.startTick) * ppt;
    ...
    static void updateVelocity(MainComponent& main, const juce::Component& piano, int mouseY)
    {
        const auto laneTop = juce::jmax(rulerHeight, piano.getHeight() - velocityLaneHeight);
    ...
    {
        if (!editing || pianoRoll == nullptr) { stopTimer(); return; }
        auto& desktop = juce::Desktop::getInstance();
