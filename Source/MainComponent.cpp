...
void MainComponent::drawTrackArea(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    for (int i = 0; i < AudioEngine::maxAudioTracks; ++i)
    {
        ...
        auto header = row.removeFromLeft(headerW);
        g.setColour(i == selectedTrack ? juce::Colour(0xff263746) : juce::Colour(0xff1e232a));
        g.fillRect(header);
        g.setColour(juce::Colours::white);
        g.setFont(juce::Font(14.0f, juce::Font::bold));
        g.drawText("Audio " + juce::String(i + 1), header.getX() + 14, header.getY() + 8, 150, 22, juce::Justification::left);
        g.setColour(i == selectedTrack ? juce::Colour(0xff9fc7e8) : juce::Colour(0xff747b85));
        g.setFont(juce::Font(10.0f));
        g.drawText(audioEngine.hasAudioFile(i) ? audioEngine.getAudioFileName(i) : "EMPTY AUDIO TRACK", header.getX() + 14, header.getY() + 36, 182, 16, juce::Justification::left, true);
        ...
        if (audioEngine.hasAudioFile(i))
        {
            ...
            if (!waveformMin[(size_t)i].empty())
            {
                ...
                const auto points = static_cast<int>(waveformMin[(size_t)i].size());
                juce::Path waveform;
            ...
            if (i == selectedTrack && clip.getWidth() >= 110)
            {
                ...
                g.drawText("DRAG TO MOVE", clip.getX() + 8, clip.getBottom() - 16, 90, 12, juce::Justification::left);
            }
        }
        else
        {
            ...
            g.fillRoundedRectangle(clip.toFloat(), 5.0f);
            g.setColour(juce::Colour(0xff505862));
            g.drawRoundedRectangle(clip.toFloat(), 5.0f, 1.0f);
            g.setColour(juce::Colour(0xff707780));
            g.setFont(juce::Font(11.0f));
            g.drawText("Select this track, then IMPORT AUDIO", clip, juce::Justification::centred);
        }
    }

    auto midiRow = rows.removeFromTop(rowH);
    auto instrumentRow = rows.removeFromTop(rowH);
    for (auto row : { midiRow, instrumentRow })
    {
        g.setColour(juce::Colour(0xff14171c));
        g.fillRect(row);
    }
    g.setColour(juce::Colour(0xff1e232a));
    g.fillRect(midiRow.removeFromLeft(headerW));
    g.fillRect(instrumentRow.removeFromLeft(headerW));
    g.setColour(juce::Colours::white);
    g.setFont(juce::Font(14.0f, juce::Font::bold));
    ...
    {
        g.setColour(juce::Colours::white);
        g.drawLine(playheadX, (float)ruler.getY(), playheadX, (float)area.getBottom(), 2.0f);
    }
}
...
void MainComponent::drawMixer(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    for (int i = 0; i < AudioEngine::maxAudioTracks + 1; ++i)
    {
        ...
        g.drawText(master ? "MASTER" : "Audio " + juce::String(i + 1), c.getX(), c.getY() + 8, c.getWidth(), 20, juce::Justification::centred);

        if (!master)
        {
            auto mute = juce::Rectangle<int>(c.getX() + 8, c.getY() + 32, 44, 20);
            auto soloButton = juce::Rectangle<int>(c.getX() + 58, c.getY() + 32, 44, 20);
            g.setColour(muted ? juce::Colour(0xff9b4545) : juce::Colour(0xff252a31));
            g.fillRoundedRectangle(mute.toFloat(), 4.0f);
            g.setColour(solo ? juce::Colour(0xff8b7a32) : juce::Colour(0xff252a31));
            g.fillRoundedRectangle(soloButton.toFloat(), 4.0f);
            g.setColour(juce::Colour(0xff454b54));
            g.drawRoundedRectangle(mute.toFloat(), 4.0f, 1.0f);
            g.drawRoundedRectangle(soloButton.toFloat(), 4.0f, 1.0f);
            g.setColour(juce::Colours::white);
        ...
        g.fillRoundedRectangle(fader.getX() - 2.0f, knobY - 6.0f, fader.getWidth() + 4.0f, 12.0f, 3.0f);
...
void MainComponent::rebuildWaveformCache(int trackIndex)
{
    ...
    waveformMin[(size_t)trackIndex].clear();
    waveformMax[(size_t)trackIndex].clear();
    ...
    auto& minCache = waveformMin[(size_t)trackIndex];
    auto& maxCache = waveformMax[(size_t)trackIndex];
    minCache.resize(points, 0.0f);
    maxCache.resize(points, 0.0f);
    ...
    for (int point = 0; point < points; ++point)
    {
        ...
        minCache[(size_t)point] = juce::jlimit(-1.0f, 1.0f, minValue);
        maxCache[(size_t)point] = juce::jlimit(-1.0f, 1.0f, maxValue);
    }
}
...
bool MainComponent::handleMixerMouse(const juce::MouseEvent& event)
{
    ...
    if (event.position.y < mixerTop) return false;
    ...
    for (int i = 0; i < AudioEngine::maxAudioTracks + 1; ++i)
    {
        ...
        if (i < AudioEngine::maxAudioTracks)
        {
            ...
            if (isMouseDown && mute.contains(event.getPosition()))
            {
                audioEngine.setTrackMuted(i, !audioEngine.isTrackMuted(i));
                repaint();
                return true;
            }
            if (isMouseDown && solo.contains(event.getPosition()))
            {
                audioEngine.setTrackSolo(i, !audioEngine.isTrackSolo(i));
                repaint();
                return true;
            }
        }
