...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        {
            const int note = lowestKey + row, y = rulerHeight + (visibleKeys - 1 - row) * keyHeight;
        ...
        for (const auto& n : owner.getMidiEngine().getNotesCopy())
        {
            ...
            const int row = (int)n.pitch - lowestKey;
            ...
            g.setColour(juce::Colour(0xff4f82ff)); g.fillRoundedRectangle(r.toFloat(), 3.0f);
            g.setColour(juce::Colour(0xff9fd8f5)); g.drawRoundedRectangle(r.toFloat(), 3.0f, 1.0f);
        }
    }
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        for (const auto& n : midi.getNotesCopy())
        {
            ...
            const int w = juce::jmax(8, (int)std::llround((double)n.lengthTicks * pixelsPerTick));
            if (n.pitch == pitch && juce::Rectangle<int>(x + 1, y, w - 2, keyHeight - 4).contains(e.getPosition()))
            {
                draggingNote = true; dragMoved = false; dragStartTick = n.startTick; dragPitch = n.pitch; dragChannel = n.channel; return;
            }
        }
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        {
            dragStartTick = newTick; dragPitch = newPitch; dragMoved = true; repaint(); owner.repaint();
        }
    }
    ...
    {
        if (!draggingNote) return;
        if (!dragMoved) owner.getMidiEngine().removeNoteAt(dragStartTick, dragPitch, dragChannel);
        draggingNote = false; dragMoved = false; repaint(); owner.repaint();
    }
...
public:
    ...
    ~MidiEditorMouseListener() override { juce::Desktop::getInstance().removeGlobalMouseListener(this); }
    void mouseDown(const juce::MouseEvent& e) override
    {
        auto* main = dynamic_cast<MainComponent*>(e.eventComponent);
