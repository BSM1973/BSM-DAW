...
namespace
{
constexpr int keyCount = 88;
constexpr int lowestKey = 21; // A0
constexpr int pianoKeyWidth = 72;
constexpr int rulerHeight = 30;
constexpr int keyHeight = 20;
constexpr int gridWidth = 1100;
constexpr int visibleKeys = 40;
constexpr std::int64_t gridTicks = MidiEngine::ticksPerQuarterNote / 4; // 16th notes
...
{
public:
    explicit PianoRoll(MainComponent& ownerIn)
        : owner(ownerIn)
    {
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        const auto keyboard = juce::Rectangle<int>(0, rulerHeight, pianoKeyWidth, bounds.getHeight() - rulerHeight);
        ...
        g.fillRect(juce::Rectangle<int>(0, 0, pianoKeyWidth, rulerHeight));
        g.setColour(juce::Colour(0xff20242b));
        g.fillRect(juce::Rectangle<int>(pianoKeyWidth, 0, grid.getWidth(), rulerHeight));

        const double pixelsPerTick = 0.12;
        const auto ticksPerMeasure = MidiEngine::ticksPerMeasure(owner.timeSignatureNumerator, owner.timeSignatureDenominator);
        ...
        for (std::int64_t tick = 0; tick <= ticksPerMeasure * 32; tick += gridTicks)
        {
            ...
            g.setColour(measure ? juce::Colour(0xff4b525c)
                                : beat ? juce::Colour(0xff343a43)
        ...
        const int highestVisible = lowestKey + std::min(keyCount, visibleKeys);
        for (int note = lowestKey; note < highestVisible; ++note)
        {
            const int row = note - lowestKey;
            const int y = rulerHeight + (visibleKeys - 1 - row) * keyHeight;
            const bool black = isBlackKey(note);
            auto key = juce::Rectangle<int>(0, y, pianoKeyWidth, keyHeight);

            g.setColour(black ? juce::Colour(0xff171a1f) : juce::Colour(0xffd6d9de));
            g.fillRect(key);
            g.setColour(black ? juce::Colour(0xff303640) : juce::Colour(0xff777d86));
            g.drawRect(key, 1);

            if (!black)
            {
                g.setColour(juce::Colour(0xff2b3037));
                g.setFont(juce::Font(9.0f));
                g.drawText(juce::String(noteName(note)) + juce::String(note / 12 - 1),
                           key.reduced(5, 0), juce::Justification::centredLeft);
            }
        }

        for (const auto& note : owner.midiEngine.getNotesCopy())
        {
            ...
            if (noteRect.getRight() <= grid.getX() || noteRect.getX() >= grid.getRight())
                continue;

            g.setColour(juce::Colour(0xff4f82ff));
            g.fillRoundedRectangle(noteRect.toFloat(), 3.0f);
            g.setColour(juce::Colour(0xff9fd8f5));
    ...
    void mouseDown(const juce::MouseEvent& event) override
    {
        ...
        const auto gridX = pianoKeyWidth;
        if (event.y < rulerHeight || event.x < gridX)
            return;

        const int row = juce::jlimit(0, visibleKeys - 1, (event.y - rulerHeight) / keyHeight);
        const int pitch = lowestKey + (visibleKeys - 1 - row);
        const double pixelsPerTick = 0.12;
        const auto rawTick = static_cast<std::int64_t>(std::llround((event.x - gridX) / pixelsPerTick));
        const auto startTick = MidiEngine::quantizeTick(juce::jmax<std::int64_t>(0, rawTick), gridTicks);
        const auto lengthTicks = MidiEngine::ticksPerQuarterNote;

        const auto existing = owner.midiEngine.getNotesCopy();
        for (const auto& note : existing)
        {
            if (note.pitch == pitch && note.startTick == startTick)
            {
                owner.midiEngine.removeNoteAt(startTick, pitch, note.channel);
                repaint();
                owner.repaint();
                return;
            }
        ...
        if (owner.midiEngine.addNote(startTick, lengthTicks, pitch, 100, 1))
        {
            repaint();
            owner.repaint();
        }
...
{
public:
    explicit MidiEditorWindow(MainComponent& ownerIn)
...
public:
    ...
    void mouseDoubleClick(const juce::MouseEvent& event) override
    {
        ...
        auto* main = dynamic_cast<MainComponent*>(component);
        if (main == nullptr)
            return;

        constexpr int top = 76 + 32 + (4 * 70);
        constexpr int rowHeight = 70;
        const auto position = event.getEventRelativeTo(main).getPosition();

        if (position.y >= top && position.y < top + rowHeight && position.x >= 0)
...
{
    midiEditorWindows.erase(
        std::remove_if(midiEditorWindows.begin(), midiEditorWindows.end(),
                       [](const auto& window) { return window == nullptr || !window->isVisible(); }),
        midiEditorWindows.end());
